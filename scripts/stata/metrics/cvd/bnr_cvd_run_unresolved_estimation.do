...
PURPOSE:     Stage 4D controller for private annual aggregate unresolved-linkage
              ...
              and writes aggregate Primary and Inclusive DCO components only.

              It does not alter candidate-level linkage, calculate metrics,
              approve, promote or publish anything.

USAGE:       do "$BNR_STATA/metrics/cvd/bnr_cvd_run_unresolved_estimation.do" 2024 04 2026 07

              To deliberately replace the private Stage 4D outputs:
...
local output_log "$BNR_PRIVATE_LOGS/bnr_cvd_unresolved_estimation_`cvd_year4'`cvd_month2'_mort_`mortality_year4'`mortality_month2'.log"
...
foreach output_file in results_output qa_output {
    ...
    if !_rc & !`replace_existing' {
        display as error "A Stage 4D private output already exists: ``output_file''"
...
capture log close stage4d
log using `"`output_log'"', text replace name(stage4d)

display as text "BNR CVD STAGE 4D: PRIVATE UNRESOLVED-LINKAGE ESTIMATION"
...
capture noisily do "$BNR_STATA/metrics/cvd/bnr_cvd_estimate_unresolved_core.do" `"`linkage_input'"' `"`results_output'"' `"`qa_output'"'
if _rc {
    local core_rc = _rc
    display as error "Stage 4D did not complete; no public output was created."
    log close stage4d
    exit `core_rc'
}
...
display as result ""
display as result "============================================================================="
display as result "STAGE 4D: PRIVATE UNRESOLVED-LINKAGE ESTIMATION"
...
display as result "  Next action:            Review aggregate estimates before DCO metric construction"
display as result "============================================================================="
