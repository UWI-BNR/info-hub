...
This amended contract incorporates the completed Stage 4A–4E-c work and the
approved Stage 3 subtype-ordering clarification. It now
...
| Annual hospital rates | Incidence / rate per 100,000 | Hospital only; observed | All CVD, Heart, Stroke / all, female, male / all and age-standardised | Annual | Exact hospital rate |
| Annual national rates | Incidence / rate per 100,000 | Hospital + DCO; Primary or Inclusive | All CVD, Heart, Stroke / all, female, male / all and age-standardised | Annual | Central rate plus linkage lower/upper values |

Initial public rate reporting is deliberately annual. It includes all, female
and male strata for hospital-only and hospital-plus-DCO rates, and it includes
only all-age crude and age-standardised public rows. Build the all-sex rate
directly from records and denominators; do not derive it by adding potentially
suppressed public sex rows. The five-year age strata used for standardisation,
age-unknown records, allocation probabilities and the mixed/unallocated
accounting category remain private.
...
8. Primary/Inclusive ordering diagnostics and rate-bound propagation in every
   identical annual stratum; subtype ordering is diagnostic because the two
   subtype classifications are not nested.
...
- Pooled All-CVD Inclusive lower, central and upper values are never below
  the corresponding Primary values in the same annual stratum.
- Heart and Stroke Primary/Inclusive ordering is reported for review but is
  not an acceptance invariant: subtype classifications are alternative
  evidence routes, and an Inclusive record may move into mixed/unallocated
  accounting rather than increase that subtype.
...
- Sex-specific DCO-enhanced rates are produced from a private sex-by-five-year
  age joint allocation; a pooled-sex DCO fraction is never applied to a
  sex-specific rate.
- The all-sex rate is calculated directly from the all-sex numerator and
  denominator, not reconstructed from public female and male rows.
