...
fields are blank, and no private support or decision fields remain.
*******************************************************************************/
...
display as result "Running expanded CVD Step 5 Stage 9 candidate-audit helper v1.1.2"
...
local key_variables metric_id period_type period_year period_month event_type sex age_group ascertainment_scope statistic mortality_definition
...
local candidate_required release_id suppression_status value numerator denominator linkage_lower_value linkage_upper_value
...
generate byte protection_failure = expected_protected == 1 & candidate_protected == 0
generate byte comparator_failure = related_suppression_review == 1 & candidate_protected == 0
generate byte numeric_failure = candidate_protected == 1 & (!missing(value) | !missing(numerator) | !missing(denominator) | !missing(linkage_lower_value) | !missing(linkage_upper_value))
generate str8 result = cond(protection_failure == 0 & comparator_failure == 0 & numeric_failure == 0, "PASS", "FAIL")
generate str244 detail = "Candidate protection and numeric blanking agree with the private register."
...
replace detail = "`numeric_failures' protected rows retain a numeric release field." in 3
replace check = "Private fields absent from candidate" in 4
replace result = cond(`private_field_count' == 0, "PASS", "FAIL") in 4
replace detail = "`private_field_count' private support or decision fields remain in the candidate." in 4
save "`qa_dta'", replace
