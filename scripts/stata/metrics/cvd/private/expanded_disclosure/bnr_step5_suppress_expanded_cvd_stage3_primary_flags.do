...
display as result "Running expanded CVD Step 5 Stage 3 primary-flag helper v0.4.0"
...
replace primary_suppression = 1 if metric_id == "CVD-INCIDENCE-001" & ascertainment_scope == "hospital_only" & stage3_hospital_primary == 1
...
replace primary_suppression = 1 if metric_id == "CVD-INCIDENCE-001" & ascertainment_scope == "hospital_only" & stage3_hospital_primary == 1
replace primary_suppression = 1 if metric_id == "CVD-INCIDENCE-001" & ascertainment_scope == "hospital_plus_dco" & stage3_dco_primary == 1
...
quietly count if metric_id == "CVD-INCIDENCE-001" & ascertainment_scope == "hospital_plus_dco" & stage3_dco_primary == 1 & primary_suppression == 1
...
replace check = "Hospital ASR flags inherited from crude numerators" in 2
replace detail = "`inherited_asr_rows' ASR rows inherited primary protection." in 2
replace check = "DCO rate flags inherited from component support" in 3
replace detail = "`dco_primary_rows' DCO-enhanced rate rows have small component support." in 3
