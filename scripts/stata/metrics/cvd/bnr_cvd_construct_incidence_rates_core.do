...
preserve
    ...
    local subtype_definition_diagnostics = r(N)
    if `subtype_definition_diagnostics' > 0 {
...
restore
post `qa_handle' ("Subtype Primary/Inclusive ordering diagnostics") ///
    (`subtype_definition_diagnostics') ///
    ("Informative only: subtype allocations are alternative classifications, not nested bounds")
...
preserve
    ...
    quietly count if estimateinclusive + 0.000001 < estimateprimary | ///
        lowerinclusive + 0.000001 < lowerprimary | ///
        upperinclusive + 0.000001 < upperprimary
    local all_definition_order_failures = r(N)
restore
post `qa_handle' ("All-CVD Primary/Inclusive ordering failures") ///
    (`all_definition_order_failures') ///
...
assert `all_definition_order_failures' == 0
