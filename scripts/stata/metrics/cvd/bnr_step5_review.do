...
foreach required_global in BNR_STATA BNR_PRIVATE BNR_STAGING BNR_PUBLIC BNR_PRIVATE_LOGS {
    if `"$`required_global'"' == "" {
...
local review_reference_csv "`review_dir'/cvd_monthly_reference_2015_2019.csv"
local review_reference_yml "`review_dir'/cvd_monthly_reference_2015_2019.yml"

local public_reference_dta ///
    "$BNR_PUBLIC/metrics/cvd/burden/cvd_monthly_reference_2015_2019.dta"
local public_reference_csv ///
    "$BNR_PUBLIC/metrics/cvd/burden/cvd_monthly_reference_2015_2019.csv"
local public_reference_yml ///
    "$BNR_PUBLIC/metrics/cvd/burden/metadata/cvd_monthly_reference_2015_2019.yml"
...
local public_reference_ready_dta ///
    "`ready_data'/cvd_monthly_reference_2015_2019.dta"
local public_reference_ready_csv ///
    "`ready_data'/cvd_monthly_reference_2015_2019.csv"
local public_reference_ready_yml ///
    "`ready_meta'/cvd_monthly_reference_2015_2019.yml"
...
if "`action'" == "prepare" {
    ...
    * while an older approved package remains beside it.
    foreach output_file in ///
            ...
            `"`public_release_yml'"' ///
            `"`public_current_yml'"' ///
            `"`public_reference_ready_dta'"' ///
            `"`public_reference_ready_csv'"' ///
            `"`public_reference_ready_yml'"' ///
            `"`public_package_yml'"' {
        capture confirm file `"`output_file'"'
        if !_rc & !`replace_existing' {
            _bnr_step5_fail 602 "`action'" "`release_id'" ///
    ...
    * local names. This is longer, but much easier to audit and hand over.
    if `replace_existing' {
        ...
        capture erase `"`public_reference_ready_dta'"'
        capture erase `"`public_reference_ready_csv'"'
        capture erase `"`public_reference_ready_yml'"'
    ...
    * releases copy the already-published asset for checksum-bound review.
    ...
    if _rc {
        local reference_rc = _rc
        local reference_reason ///
            "The monthly reference asset could not be prepared (Stata return code `reference_rc')."
        _bnr_step5_fail `reference_rc' "`action'" "`release_id'" ///
    ...
    * rows that require suppression. It also creates a machine-readable QA file.
    ...
    export delimited using `"`review_equation_audit'"', replace
    ...
    * loop but is easier to audit and avoids fragile macro expansion.
    ...
    quietly checksum `"`review_equation_audit'"'
    ...
    quietly checksum `"`review_reference_csv'"'
    post `basis_handle' ("monthly_reference_csv") ///
        (`"`review_reference_csv'"') (r(filelen)) (r(checksum))

    quietly checksum `"`review_reference_yml'"'
    post `basis_handle' ("monthly_reference_metadata") ///
        (`"`review_reference_yml'"') (r(filelen)) (r(checksum))
    ...
    * -----------------------------------------------------------------------
    ...
    replace review_item = "Monthly reference" in 14
...
if "`action'" == "approve" {
    ...
    * every check.
    ...
    if r(N) {
        _bnr_step5_fail 459 "`action'" "`release_id'" `"`output_log'"' ///
            "The reviewed candidate is not the approved Stage 2 hospital-only schema."
        exit _rc
    }
    ...
    if r(N) {
        _bnr_step5_fail 459 "`action'" "`release_id'" `"`output_log'"' ///
            "The reviewed candidate contains a monthly row outside the approved lattice."
        exit _rc
    }
    ...
    * explicit mappings so their position in the basis file remains auditable.
    forvalues basis_row = 6/9 {
        if `basis_row' == 6 {
            ...
            local expected_role "monthly_reference_metadata"
            local expected_path `"`review_reference_yml'"'
        }
        local recorded_role = file_role[`basis_row']
        local recorded_path = file_path[`basis_row']
    ...
    * after all review-basis and disclosure checks have passed.
    ...
    foreach dataset_type in release current {
        ...
        file write `dataset_meta' "schema: bnr_cvd_public_metric_v2" _n
        ...
        file write `dataset_meta' "ascertainment_scope: hospital_only" _n
        file write `dataset_meta' "mortality_definition: not_applicable" _n
        file write `dataset_meta' "estimate_basis: observed" _n
        ...
        file write `dataset_meta' "monthly_historical_reference: cvd_monthly_reference_2015_2019" _n
    ...
    file open `package_meta' using `"`metadata_package'"', ///
        write text replace
    file write `package_meta' "schema: bnr_cvd_public_metric_package_v2" _n
    ...
    file write `package_meta' "monthly_reference_asset: cvd_monthly_reference_2015_2019" _n
    file write `package_meta' "monthly_reference_rule: checksum_verify_do_not_recalculate" _n
    ...
    * Copy the ten approved files explicitly. This deliberately avoids
    * nested macro indirection: the longer form is easier to audit and hand over.
    ...
    local ready_outputs public_release_dta public_release_csv ///
        ...
        public_current_yml public_package_yml public_reference_ready_dta ///
        public_reference_ready_csv public_reference_ready_yml
    ...
    if !`copy_rc' {
        capture copy `"`review_reference_dta'"' `"`public_reference_ready_dta'"', replace
        if _rc {
            local copy_rc = _rc
            local failed_target `"`public_reference_ready_dta'"'
        }
    }

    if !`copy_rc' {
        capture copy `"`review_reference_csv'"' `"`public_reference_ready_csv'"', replace
        if _rc {
            local copy_rc = _rc
            local failed_target `"`public_reference_ready_csv'"'
        }
    }

    if !`copy_rc' {
        capture copy `"`review_reference_yml'"' `"`public_reference_ready_yml'"', replace
        if _rc {
            local copy_rc = _rc
            local failed_target `"`public_reference_ready_yml'"'
        }
    }
    ...
    * Manifest only the files Step 6 may promote.
    local manifest_files ///
        ...
        datasets/cvd_monthly_reference_2015_2019.dta ///
        datasets/cvd_monthly_reference_2015_2019.csv ///
