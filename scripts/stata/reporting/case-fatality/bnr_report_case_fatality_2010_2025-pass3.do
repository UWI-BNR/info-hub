...
Version: 0.3.1 (23 September 2026)

CURRENT STAGE: frozen-method diagnostic and public-layout planning. The primary
outcome is death within 30 days identified by either deterministic mortality
linkage or a valid hospital death record. Linked-only and in-hospital measures
are secondary. This file still stops before final metrics, disclosure approval,
the shareable dataset or PDF. Extend this same file after private review.
...
*******************************************************************************/
local study_id "cvd_case_fatality_2010_2025_v02"
...
rename dth_date candidate_dth_date
rename final_linkage_rule_id candidate_linkage_rule
...
if r(N) == 0 {
    ...
    generate int days_to_death = candidate_dth_date-doe
    ...
    * record per event.
    ...
    preserve
        ...
        generate int death_year = year(candidate_dth_date)
    ...
    sort eid days_to_death candidate_dth_date death_record_id
    by eid: keep if _n == 1
    keep eid death_record_id candidate_dth_date days_to_death ///
        candidate_linkage_rule
    rename death_record_id record_id
    rename candidate_dth_date dth_date
    rename candidate_linkage_rule final_linkage_rule_id
    isid eid
    save `"`event_deaths'"', replace
}
...
generate byte primary_death_30 = linked_death_30 == 1 | hospital_death_30 == 1

assert primary_death_30 == ///
    both_death_sources + linked_only_death + hospital_only_death
assert one == primary_death_30 + neither_death_source
...
preserve
    keep if linked_death_30 == 1
    generate byte one_linked_outcome = 1
    collapse (sum) linked_deaths=one_linked_outcome, ///
        by(event_year etype sex final_linkage_rule_id)
    sort event_year etype sex final_linkage_rule_id
...
preserve
    ...
    generate byte one_linked_outcome = 1
    collapse (sum) linked_deaths=one_linked_outcome, ///
        by(event_year etype sex days_to_death final_linkage_rule_id)
    sort event_year etype sex days_to_death final_linkage_rule_id
...
generate int candidate_days_to_death = candidate_dth_date-doe if ///
...
/*******************************************************************************
CONTROLLED SECTION 7 — PROVISIONAL PUBLIC-LAYOUT DISCLOSURE AUDIT
DO NOT EDIT. This is not a public dataset. It tests a layout designed to retain
annual Heart/Stroke-by-sex primary results while placing secondary measures at
a broader grain so small source-only counts cannot be recovered by subtraction.
*******************************************************************************/
use "`review_root'/linked_index_events_private.dta", clear

* Primary: annual Heart/Stroke results for women, men and both sexes.
* Unknown-sex counts are retained only as private disclosure checks because
* combined-sex minus the two displayed sex strata would otherwise reveal them.
preserve
    generate byte unknown_sex_event = !inlist(sex,1,2)
    generate byte unknown_sex_death = unknown_sex_event*primary_death_30
    generate byte unknown_sex_noncase = ///
        unknown_sex_event*(1-primary_death_30)
    collapse (sum) unknown_sex_denominator=unknown_sex_event ///
        unknown_sex_numerator=unknown_sex_death ///
        unknown_sex_noncase, by(event_year etype)
    save `"`primary_unknown_sex'"', replace
restore
preserve
    keep if inlist(sex,1,2)
    collapse (sum) denominator=one numerator=primary_death_30, ///
        by(event_year etype sex)
    generate int period_start = event_year
    generate int period_end = event_year
    generate str20 period_label = string(event_year,"%04.0f")
    generate str20 time_basis = "annual"
    generate str32 measure_id = "primary_combined_30d"
    generate str40 source_quality_status = "candidate"
    generate str48 interpretation_flag = ""
    replace interpretation_flag = "2024_abstraction_completeness" if ///
        event_year == 2024
...
* Secondary comparisons: broad periods, both event types and both sexes. The
* complement, shared-source and noncase counts are audited because subtraction
* among the released measures must not reconstruct a count of 1--5.
generate int period_start = .
generate int period_end = .
replace period_start = 2010 if inrange(event_year,2010,2015)
replace period_end   = 2015 if period_start == 2010
replace period_start = 2016 if inrange(event_year,2016,2020)
replace period_end   = 2020 if period_start == 2016
replace period_start = 2021 if inrange(event_year,2021,2023)
replace period_end   = 2023 if period_start == 2021
replace period_start = 2024 if inrange(event_year,2024,2025)
replace period_end   = 2025 if period_start == 2024

preserve
    collapse (sum) denominator=one numerator=linked_death_30 ///
        complement_n=hospital_only_death, by(period_start period_end)
    generate int event_year = .
    generate byte etype = 0
    generate byte sex = 0
    generate str20 period_label = string(period_start,"%04.0f") + "-" + ///
        string(period_end,"%04.0f")
    generate str20 time_basis = "broad_period"
    generate str32 measure_id = "secondary_linked_30d"
    generate str40 source_quality_status = "candidate"
    generate str48 interpretation_flag = ""
    replace interpretation_flag = "2024_abstraction_completeness" if ///
        period_start == 2024
...
restore
preserve
    collapse (sum) denominator=one numerator=hospital_death_30 ///
        complement_n=linked_only_death shared_source_n=both_death_sources, ///
        by(period_start period_end)
    generate int event_year = .
    generate byte etype = 0
    generate byte sex = 0
    generate str20 period_label = string(period_start,"%04.0f") + "-" + ///
        string(period_end,"%04.0f")
    generate str20 time_basis = "broad_period"
    generate str32 measure_id = "secondary_in_hospital_30d"
    generate str40 source_quality_status = "candidate"
    replace source_quality_status = "not_release_source_quality" if ///
        inlist(period_start,2010,2021)
    generate str48 interpretation_flag = ""
    replace interpretation_flag = "extreme_dod_values" if period_start == 2010
    replace interpretation_flag = "2023_missing_discharge_status" if ///
        period_start == 2021
    replace interpretation_flag = "2024_abstraction_completeness" if ///
        period_start == 2024
...
generate double complement_n = .
generate double shared_source_n = .
append using `"`secondary_linked'"' `"`secondary_hospital'"'
generate double case_fatality_pct = 100*numerator/denominator
generate double noncase_n = denominator-numerator
generate byte small_denominator = inrange(denominator,1,5)
generate byte small_numerator = inrange(numerator,1,5)
generate byte small_complement = inrange(complement_n,1,5)
generate byte small_shared_source = inrange(shared_source_n,1,5)
generate byte small_noncase = inrange(noncase_n,1,5)
generate byte small_unknown_sex = ///
    inrange(unknown_sex_denominator,1,5) | ///
    inrange(unknown_sex_numerator,1,5) | ///
    inrange(unknown_sex_noncase,1,5)
generate byte candidate_release = source_quality_status == "candidate" & ///
    small_denominator == 0 & small_numerator == 0 & ///
    small_complement == 0 & ///
    small_shared_source == 0 & small_noncase == 0
replace candidate_release = 0 if time_basis == "annual" & sex == 0 & ///
    small_unknown_sex == 1
format case_fatality_pct %6.2f
order measure_id time_basis period_label period_start period_end event_year ///
    etype sex denominator numerator case_fatality_pct complement_n ///
    shared_source_n noncase_n unknown_sex_denominator ///
    unknown_sex_numerator unknown_sex_noncase candidate_release ///
    source_quality_status interpretation_flag
sort measure_id period_start event_year etype sex
save "`review_root'/candidate_public_layout_disclosure_audit.dta", replace

quietly count if candidate_release == 1 & ///
    (small_denominator == 1 | small_numerator == 1 | ///
    small_complement == 1 | ///
    small_shared_source == 1 | small_noncase == 1)
assert r(N) == 0
quietly count if candidate_release == 1 & time_basis == "annual" & ///
    sex == 0 & small_unknown_sex == 1
assert r(N) == 0
...
display as result "Frozen-method diagnostic and public-layout audit complete."
display as text "Review private files in `review_root'."
display as text "No public dataset or PDF was created; candidate layout rows still require human disclosure review."
log close case_fatality
