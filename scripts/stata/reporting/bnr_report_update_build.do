...
VERSION: 0.1.2 (2 September 2026)
...
local report_id "cvd_update_`report_year4'_`report_month2'_v`version_num'"
local report_dir "$BNR_REPO/site/surveillance/cvd/reports/updates/`report_period'"
local report_page "`report_dir'/index.qmd"
...
foreach source_file in event_csv mortality_csv {
    capture confirm file "``source_file''"
    if _rc {
        display as error "Required release-stamped public CSV not found: ``source_file''"
        exit 601
    }
}
...
capture confirm file "`report_page'"
if !_rc & !`replace_existing' {
    display as error "A report already exists for `report_period'."
    display as error "Use a higher report version and the replace option for a correction."
    exit 602
}
...
noisily display as result "============================================================================="
noisily display as result "ROLLING THREE-MONTH CVD UPDATE: BUILD SUMMARY"
noisily display as text   "  Run status:              Page created"
noisily display as text   "  Script version:          0.1.2"
...
noisily display as text  `"  Output page:             `report_page'"'
