...
- Candidate and component DTAs are committed only after embedded acceptance QA
  succeeds.
- Subtype Primary/Inclusive ordering is a diagnostic rather than a hard
  acceptance rule. Pooled All-CVD ordering remains hard. In the production
  diagnostic, two 2012 Heart female strata had slightly lower Inclusive upper
  rates than Primary (82.393600 vs 82.668596 for all-age; 48.690048 vs
  48.826391 age-standardised), with central and lower values not materially
  affected. This is expected under alternative, non-nested subtype
  classifications and no value was forced with max().
...
| Definitions | Subtype ordering is diagnostic; pooled All-CVD Inclusive lower/central/upper must not fall below Primary in an identical published stratum. |
...
| Version control | Every delivered DO-file header and test reports release 1.0.7; manifest checksums are regenerated from final bytes. |
