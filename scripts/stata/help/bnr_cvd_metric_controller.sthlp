...
{p 4 4 2}
{cmd:db bnr_cvd_metric_controller} opens the Step 4 dialog. It calculates the
selected implemented metric families from a completed Step 3 release and creates
a private staging package for human review.

{title:Syntax}

{p 8 8 2}
{cmd:do "$BNR_STATA/monthly/bnr_cvd_metric_controller.do"} {it:year} {it:month} {it:metric_family} [{it:metric_family} ...] [{cmd:replace}]
...
{p 8 8 2}
{cmd:. do "$BNR_STATA/monthly/bnr_cvd_metric_controller.do" 2024 3 burden}

{p 8 8 2}
To replace the existing selected private staging package deliberately:
{cmd:. do "$BNR_STATA/monthly/bnr_cvd_metric_controller.do" 2024 3 burden replace}
...
{p 4 4 2}
If the run cannot continue, the final block is {bf:STEP 4 DID NOT COMPLETE}.
