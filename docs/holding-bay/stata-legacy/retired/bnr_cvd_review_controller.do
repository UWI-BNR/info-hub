/*******************************************************************************
DO-FILE:     bnr_cvd_review_controller.do
VERSION:     1.5.1 (23 July 2026)
PROJECT:     BNR Refit Phase 2
WORKFLOW:    Step 5 - prepare, review and record approval

PURPOSE:     Prepare a private human-review workbook and disclosure-controlled
             candidate, or approve that exact reviewed candidate and create
             public_ready. This controller never promotes or publishes.

PREPARE:     do "$BNR_STATA/monthly/bnr_cvd_review_controller.do" ///
                 2024 2 burden prepare

             do "$BNR_STATA/monthly/bnr_cvd_review_controller.do" ///
                 2024 2 burden prepare replace

APPROVE:     do "$BNR_STATA/monthly/bnr_cvd_review_controller.do" ///
...
* A single failure reporter is the last text written to the screen and log
...
program define _bnr_step5_fail
    ...
    noisily display as error "============================================================================="
    noisily display as error "STEP 5 DID NOT COMPLETE"
    ...
    noisily display as text  "No approval, public output or website file was created."
    noisily display as error "============================================================================="
...
program define _bnr_step5_write_approval
    ...
    file write `approval_yml' ///
        "public_ready_manifest: review/public_manifest.csv" _n
...
if "`action'" == "prepare" {
    ...
    if !inlist(`"`role_lower'"', "bnr lead", "bnr analyst", ///
            "bnr statistician") {
        display as error ///
            "Approver role must be BNR Lead, BNR Analyst or BNR Statistician."
        exit 198
    }
    ...
    if `"`role_lower'"' == "bnr lead" {
        ...
        local approver_role "BNR Statistician"
    }
}
...
local public_release_dta "`ready_data'/release.dta"
local public_release_csv "`ready_data'/release.csv"
local public_current_dta "`ready_data'/current.dta"
local public_current_csv "`ready_data'/current.csv"
local public_release_yml "`ready_meta'/release.yml"
local public_current_yml "`ready_meta'/current.yml"
local public_package_yml "`ready_meta'/package.yml"
local disclosure_qa "`ready_dir'/disclosure_qa.csv"
local manifest "`review_dir'/public_manifest.csv"
local approval "`review_dir'/approval.yml"
local output_log ///
...
noisily display as result "  Script version:   1.5.0"
...
* The action branches set these values only after every required file has been
* created and checked. One shared operational summary is printed below both
* branches, immediately before the log is closed.
...
if "`action'" == "prepare" {
    ...
    local prepare_outputs review_candidate review_disclosure_qa ///
        review_basis review_workbook

    foreach output_name of local prepare_outputs {
    ...
    local protected_outputs approval public_release_dta public_release_csv ///
        public_current_dta public_current_csv public_release_yml ///
        public_current_yml public_package_yml disclosure_qa manifest

    foreach output_name of local protected_outputs {
        local output_file `"``output_name''"'
        capture confirm file `"`output_file'"'
        if !_rc & !`replace_existing' {
            _bnr_step5_fail 602 "`action'" "`release_id'" ///
    ...
    * A deliberate prepare replacement invalidates the previous Step 5 state.
    if `replace_existing' {
        foreach output_name in approval manifest review_candidate ///
                review_disclosure_qa review_basis review_workbook ///
                public_release_dta public_release_csv public_current_dta ///
                public_current_csv public_release_yml public_current_yml ///
                public_package_yml disclosure_qa {
            local output_file `"``output_name''"'
    ...
    tempfile step5_qa
    capture quietly do "$BNR_STATA/common/bnr_apply_suppression.do" ///
...
if "`action'" == "approve" {
    ...
    local ready_outputs public_release_dta public_release_csv ///
        public_current_dta public_current_csv public_release_yml ///
        public_current_yml public_package_yml disclosure_qa manifest

    foreach output_name of local ready_outputs {
        local output_file `"``output_name''"'
        capture confirm file `"`output_file'"'
        if !_rc {
            _bnr_step5_fail 602 "`action'" "`release_id'" ///
    ...
    * Manifest only the files Step 6 may promote.
    local manifest_files datasets/release.dta datasets/release.csv ///
        datasets/current.dta datasets/current.csv metadata/release.yml ///
        metadata/current.yml metadata/package.yml disclosure_qa.csv

    tempfile manifest_dta
    tempname manifest_handle
    postfile `manifest_handle' str80 relative_path str12 file_type ///
        double file_size double checksum using `"`manifest_dta'"', replace
...
* ---------------------------------------------------------------------------
* This uses a temporary frame so the current dataset is not altered. It is
* deliberately outside PREPARE and APPROVE, making it the final substantive
* section in both the Results window and the private log.
tempname step5_summary_frame
frame create `step5_summary_frame'

if "`action'" == "prepare" {
    frame `step5_summary_frame': quietly set obs 10
    frame `step5_summary_frame': generate str30 operational_item = ""
    frame `step5_summary_frame': generate strL result = ""
    frame `step5_summary_frame': replace operational_item = "Run status" in 1
    frame `step5_summary_frame': replace result = "`summary_status'" in 1
    frame `step5_summary_frame': replace operational_item = "Release" in 2
    frame `step5_summary_frame': replace result = "`release_id' (burden)" in 2
    frame `step5_summary_frame': replace operational_item = "Action" in 3
    frame `step5_summary_frame': replace result = "Prepare" in 3
    frame `step5_summary_frame': replace operational_item = "OPEN THIS FILE FIRST" in 4
    frame `step5_summary_frame': replace result = `"`summary_main_file'"' in 4
    frame `step5_summary_frame': replace operational_item = "Public candidate" in 5
    frame `step5_summary_frame': replace result = `"`summary_candidate'"' in 5
    frame `step5_summary_frame': replace operational_item = "Disclosure QA" in 6
    frame `step5_summary_frame': replace result = `"`summary_qa'"' in 6
    frame `step5_summary_frame': replace operational_item = "Review record" in 7
    frame `step5_summary_frame': replace result = `"`summary_basis'"' in 7
    frame `step5_summary_frame': replace operational_item = "Public ready" in 8
    frame `step5_summary_frame': replace result = "`summary_ready'" in 8
    frame `step5_summary_frame': replace operational_item = "Next step" in 9
    frame `step5_summary_frame': replace result = "`summary_next'" in 9
    frame `step5_summary_frame': replace operational_item = "Private log" in 10
    frame `step5_summary_frame': replace result = `"`output_log'"' in 10
}
else {
    frame `step5_summary_frame': quietly set obs 9
    frame `step5_summary_frame': generate str30 operational_item = ""
    frame `step5_summary_frame': generate strL result = ""
    frame `step5_summary_frame': replace operational_item = "Run status" in 1
    frame `step5_summary_frame': replace result = "`summary_status'" in 1
    ...
    frame `step5_summary_frame': replace result = "`release_id' (burden)" in 2
    frame `step5_summary_frame': replace operational_item = "Action" in 3
    frame `step5_summary_frame': replace result = "Approve" in 3
    frame `step5_summary_frame': replace operational_item = "Approver" in 4
    frame `step5_summary_frame': replace result = ///
        `"`approver_name' (`approver_role')"' in 4
    frame `step5_summary_frame': replace operational_item = "Public-ready package" in 5
    frame `step5_summary_frame': replace result = `"`summary_ready'"' in 5
    frame `step5_summary_frame': replace operational_item = "Public manifest" in 6
    frame `step5_summary_frame': replace result = `"`summary_manifest'"' in 6
    frame `step5_summary_frame': replace operational_item = "Approval record" in 7
    frame `step5_summary_frame': replace result = `"`summary_approval'"' in 7
    frame `step5_summary_frame': replace operational_item = "Next step" in 8
    frame `step5_summary_frame': replace result = "`summary_next'" in 8
    frame `step5_summary_frame': replace operational_item = "Private log" in 9
    frame `step5_summary_frame': replace result = `"`output_log'"' in 9
}

frame `step5_summary_frame': label variable operational_item "Operational item"
frame `step5_summary_frame': label variable result "Result"
frame `step5_summary_frame': format operational_item %-30s
frame `step5_summary_frame': format result %-210s
...
frame `step5_summary_frame': noisily list operational_item result, noobs clean abbreviate(30)
noisily display as result "============================================================================="
set linesize `original_linesize'
frame drop `step5_summary_frame'
